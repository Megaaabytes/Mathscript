...
using Math_Script_Runtime_Environment.Operations;
using Math_Script_Runtime_Environment.Exceptions;
        ...
        public static void BeginRead(Instructions[] instructions)
        {
            for (int i = 0; i < instructions.Length; i++)
            {
                ...
                else if (instruction.type == InstructionType.Endf)
                {
                    ...
                    if (FunctionList.functions.Count == 0)
                    {
                        if (tryBlockActive == true)
                        {
                            continue;
                        ...
                        {
                            throw new ScriptException($"endf() (EXPECTED ACTIVE FUNCTION BEFORE DECLARATION) AT {i}");
                        }
                    }

                    try
                    {
                        FunctionCall function = FunctionList.functions[FunctionList.functions.Count - 1];
                        i = function.lastLine;
                        FunctionList.functions.RemoveAt(FunctionList.functions.Count);
                    }
                    catch
                    {
                        try
                        {
                            FunctionCall function = FunctionList.functions[0];
                            i = function.lastLine;
                            FunctionList.functions.RemoveAt(0);
                        }
                        catch (Exception ex)
                        {
                            throw new CatastrophicException("Uncatchable error: " + ex.Message);
                        }
                    }
                }
                else if (instruction.type == InstructionType.Run)
                {
                    ...
                    {
                        FunctionList.functions.Add(new FunctionCall(functionToExecute, i));
                ...
                else if (instruction.type == InstructionType.ForIn)
                {
                    ...
                    if (array == null)
                    {
                        VariableInformation info = ReadVariable(args[0], i, variables.ToArray(), arrayVariables.ToArray());

                        if (info.var.type != VariableType.String)
                        {
                            if (tryBlockActive == true)
                            {
                                continue;
                            ...
                            {
                                throw new ScriptException($"for(?) (ARRAY VARIABLE DOES NOT EXIST OR INPUT VARIABLE IS NOT IN THE CORRECY FORMAT) AT {i}");
                            }
                        }
                        else
                        {
                            array = new ArrayVariable(info.value.Length, VariableType.Char, info.name);

                            foreach (char ch in info.value)
                            {
                                array.Add(new Variable("", VariableType.Char, ch.ToString()), i);
                            }
                        }
                    }
